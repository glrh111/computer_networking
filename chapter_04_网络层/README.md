...
以上路由选择讨论忽略了如下现实情况：
+ 规模：路由数目规模巨大，设计路由信息计算、存储及通信的开销高得不可实现。
+ 管理自治：某机构组织要求按照自己的意愿运行路由器，或对外隐藏其网络的内部组织面貌

这两个问题都可以通过将路由器组织进自治系统 Autonomous System AS 中来解决，每个AS通常由一组通常处在相同管理控制下的路由器组成。如由相同的ISP运营，或相同的公司网络。
在相同的AS中的路由器，全部运行同样的路由选择算法（LS或者DV），且拥有彼此的信息，这就像在前一节中所讲的理想化模型中的情况一样。
在一个自治系统内运行的路由算法叫做自制系统内部路由选择协议 intra-autonomous system routing protocol . 

AS互联是必须的，因此在一个AS内的一台或多台路由器将有另外的任务，即负责向本AS之外的目的地转发分组。这些路由器被称为网关路由器 gateway router .

但是在AS某一个路由器，如何将packet路由选择到位于该AS外部的目的地呢？如果仅有一台网关，这很容易决定。如果由多台网关路由器，那么该AS需要知道：
+ 相连AS的可达与不可达目的地。永远可达？
+ 向AS1中的路由器传播这些可达信息，这样每台路由器可以通过设置转发表来处理AS目的地。

从相邻AS获取可达性信息和向该AS中所有路由器传播可达性信息是两项由自治系统间路由选择协议 inter-autonomous system routing protocol 处理的任务。
因为自治系统间路由选择协议涉及两个AS之间的通信，这两个通信的AS必须运行相同的自治系统路由选择协议。
因特网中的所有AS中都运行相同的AS间路由选择协议，称为BGP4.

![每台路由器接收来自一个AS内部路由选择协议和一个AS间路由选择协议的信息](http://o9hjg7h8u.bkt.clouddn.com/4-32%E8%87%AA%E6%B2%BB%E7%B3%BB%E7%BB%9F.png)

AS与ISP的关系，ISP中的路由器一般划分为一个或多个AS。

两种AS内部路由选择协议：RIP，OSPF，与一种AS间路由选择协议 BGP。


## 4.6 因特网间的路由选择协议

RIP 用于下层ISP中，OSPF和IS-IS 通常用于上层ISP中。

### 4.6.1 因特网中自治系统内部的路由选择 RIP

AS内部路由选择协议又称为内部网协议 interior gateway protocol。

历史上曾有两个路由选择协议被广泛采用：
+ 路由选择信息协议 Routing Information Protocol 
+ 开放最短路优先 Open Shortest Path First . 与之密切相关的是IS-IS协议

RIP是一种距离向量协议，每条链路的费用为1，一条路径的最大费用被限制为15，因此RIP的使用限制在网络直径不超过15的自治系统内。

在RIP中，更新信息在邻居之间使用一种RIP相应报文 RIP response message 来交换，大概30s相互交换一次。响应报文又被称为RIP通告 RIP advertisement

每台路由器维护着一张称为路由选择表 routing table 的RIP表。

如果一台路由器，超过180秒没有听到邻居的报文，则该邻居不再被认为是可达的。这种情况下，RIP修改本地路由选择表，然后向相邻路由器发送通告来传播该信息。

路由器也可以通过使用RIP请求报文，请求其邻居到指定目的地的费用。路由器在UDP上使用520端口互相发送RIP请求与响应报文。

![作为routed守护进程的RIP实现](http://o9hjg7h8u.bkt.clouddn.com/4-39%E4%BD%9C%E4%B8%BArouted%E5%AE%88%E6%8A%A4%E7%A8%8B%E5%BA%8F%E7%9A%84RIP%E5%AE%9E%E7%8E%B0.png)

### 4.6.2 因特网中自治系统内部路由选择协议 OSPF

OSPF v2 RFC2328

Open代表公众可用，与之相对的是Cisco的EIGRP。

OSPF被设想为RIP的后继者。使用OSPF时，路由器向自制系统内所有其他路由器广播路由选择信息，而不仅仅是向其相邻路由器广播。
每当一条链路的状态发生变化时（费用变化，或连接状态变化），路由器就会广播链路状态信息。即使链路状态未发生变化，它也要周期性地广播链路状态。至少每隔30分钟一次。

OSPF报文直接承载在IP里，上层协议的值为89. 以下为OSPF的优点：
+ 安全。能够鉴别路由器之间的交换。仅有受信任的路由器能够参与一个AS内的OSPF协议。
+ 多条相同费用的路径。费用相同时，允许使用多条路径。
+ 对单播和多播路由选择的支持。
+ 支持在单个路由选择域内的层次结构。具有按层次结构构造一个自治系统的能力。

一个OSPF自治系统可以配置成多个区域，每个区域都运行自己的OSPF链路状态路由选择算法，一个区域内的每台路由器都向该区域内的所有其他路由器广播其链路状态。
在一个区域内，一台或多台区域边界路由器 area border router 负责为流向该区域以外的分组提供路由选择。最后，在AS内只有一个OSPF区域配置为主干backbone区域。
主干区域的主要作用是为AS内其他区域之间的流量提供路由选择。。。。什么jb翻译，有歧义。

流量 - 边界路由 - 主干 - 边界路由 - 流量

### 4.6.3 自治系统间的路由选择

RFC4271 定义了边界网关协议 Broder Gateway Protocol BGP . BGP4为每个AS提供了进行一下工作的手段：
+ 从相邻AS处获得子网可达信息
+ 向本AS内部的所有路由器传播这些可达性信息
+ 基于可达性信息和AS策略，决定到达子网的好路由

BGP使得每个子网向因特网的其余部分通告它的存在。

BGP极其复杂。

~[eBGP,iBGP](http://o9hjg7h8u.bkt.clouddn.com/4-40eBGP%E5%92%8CiBGP.png)

#### 1 BGP基础

在BGP中，路由器对通过使用179端口的半永久TCP连接来交换路由选择信息。

跨越两个AS的BGP session称为外部BGP(eBGP)，在同一个AS中两台路由器之间的BGP session称为内部BGP(iBGP).

BGP使得每个AS知道经过其相邻AS可达那些目的地。在BGP中，目的地不是主机，而是CDIR化的前缀preffix，每个前缀表示一个子网或子网的集合。

#### 2 路径属性和BGP路由

在BGP中，一个AS由全局唯一的AS号标识，Autonomous System Number ASN(RFC1930). 
但是stub（桩）AS没有ASN，这种SAS仅承载源地址或目的地址为本AS的流量。ASN由ICANN地区注册机构分配。

当一台路由器通过BGP session通告一个前缀时，它在前缀中包括一些BGP属性, 用BGP术语来说，带有属性的前缀被称为一条路由router。
因此，BGP peers彼此通告路由。两个较为重要的属性是，AS-PATH和NEXT-HOP。
+ AS-PATH: 包含了前缀的通告已经通过了那些AS。用来检测和防止循环通告。
+ NEXT-HOP: 是一个开始某AS-PATH的接口。路由器使用这个属性来正确配置他们的转发表。因为每个链路连接这一个interface，所以可以用来确定某条特殊的链路。

#### 3 BGP路由选择

如果路由器得知到达一个前缀的多条路由，BGP按照如下规则消除：
+ 路由器被指派一个本地偏好值。具有本地最高偏好值的路由将被选择。
+ 具有最短AS-PATH的路由将被选择
+ 最靠近NEXT-HOP路由器的路由。被称为热土豆路由选择
+ 使用BGP标识符选择
...
N次单播 N-way-unicast 可以实现广播。缺点是
+ 效率低
+ 需要通过某种渠道知道接收方地址
+ 链路状态路由选择协议LS 使用广播来分发用于计算单播路由的链路状态信息。

![源复制与网络中复制的比较](http://o9hjg7h8u.bkt.clouddn.com/4-43%E6%BA%90%E5%A4%8D%E5%88%B6%E4%B8%8E%E7%BD%91%E7%BB%9C%E4%B8%AD%E5%A4%8D%E5%88%B6%E7%9A%84%E6%AF%94%E8%BE%83.png)

广播实现的方法
+ 无控制洪范 flooding 源节点向它的所有邻居节点发送packet的副本。致命缺点，如果网络结构中有圈，那么packet将无限循环；容易导致网络中packet副本过多。
+ 受控洪范 明智决定何时洪范，何时不洪范。
  + 序号控制洪范 sequence-number-controlled flooding 源节点将地址和广播序号放入packet，再发送。这样每个节点就可以判断是否转发过。
  + 反向路径转发 Reverse Path Forwarding 有时也成为反向路径广播RPB。当一台路由器接收到具有给定源地址的广播分组时，
仅向该分组达到的链路正好是位于它自己的返回其源的最短单播路径时上，才向其所有出链路（除了接收packet的）传输报文。
